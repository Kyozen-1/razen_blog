...
<script>
    const settings = new Settings({
        @if (Auth::guard('razen_blog')->check())
            attributes: {
                color: "{{Auth::guard('razen_blog')->user()->color_layout?Auth::guard('razen_blog')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('razen_blog')->user()->nav_color?Auth::guard('razen_blog')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('razen_blog')->user()->behaviour?Auth::guard('razen_blog')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('razen_blog')->user()->layout?Auth::guard('razen_blog')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('razen_blog')->user()->radius?Auth::guard('razen_blog')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('razen_blog')->user()->placement?Auth::guard('razen_blog')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('razen-blog.admin.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
