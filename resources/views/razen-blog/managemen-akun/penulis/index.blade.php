...
@section('title', 'Razen Blog | Managemen Akun | Penulis')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Penulis</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-blog.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="#">Managemen Akun</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-blog.managemen-akun.penulis.index') }}">Penulis</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="penulis_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="penulis_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-8 col-12">
                                ...
                                <div class="mb-3 position-relative form-group" id="no_hp_form">
                            ...
                            <div class="col-md-4 col-12">
                                <div class="mb-3 position-relative form-group">
                                    ...
                                    <input type="file" class="dropify" name="foto" id="foto" data-height="150" data-allowed-file-extensions="png jpg jpeg webp" data-show-errors="true">
                                </div>
                            </div>
                        </div>
                </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-8 col-12">
                            ...
                            <div class="form-group mb-3 position-relative">
                                <label for="detail_no_hp" class="control-label">No HP</label>
                                <input type="text" id="detail_no_hp" class="form-control" disabled>
                            </div>
                            <div class="form-group mb-3 position-relative">
                                <label for="detail_email" class="control-label">Email</label>
                                <input type="text" id="detail_email" class="form-control" disabled>
                            </div>
                        </div>
                        <div class="col-md-4 col-12">
                            <div class="form-group mb-3 position-relative">
                                <img class="img-responsive" id="detail_foto">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div id="updatePasswordModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="updatePasswordModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="updatePasswordModalLabel">Update Password</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <span id="form_result"></span>
                    <form class="form-horizontal" id="password_penulis_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="penulis_id" id="password_penulis_id">
                        <div class="row">
                            <div class="col-12">
                                <div class="mb-3 position-relative form-group">
    ...
    <div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="confirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4 class="text-center" style="margin: 0;">Apakah anda yakin mengubah status penulis?</h4>
                </div>
                <div class="modal-footer">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#penulis_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-blog.managemen-akun.penulis.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            ...
            $('#aksi').val('Save');
            $('#form_result').html('');
            var add_akun_form = $(`<div id="add_akun_form">
                                        <div class="mb-3 position-relative form-group">
            ...
            $('#no_hp_form').after(add_akun_form);
        });

        $('#penulis_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-blog.managemen-akun.penulis.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#penulis_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#penulis_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-blog.managemen-akun.penulis.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-blog.managemen-akun.penulis.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_foto').attr('src', "{{ asset('images/razen-blog/foto') }}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-blog.managemen-akun.penulis.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    $('#add_akun_form').remove();
                    $('#nama').val(data.result.nama);
                    $('#no_hp').val(data.result.no_hp);

                    var lokasi_foto = "{{ asset('images/razen-blog/foto') }}"+'/'+data.result.foto;
        ...
        var user_id;
        $(document).on('click', '.status-penulis', function(){
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-blog.managemen-akun.penulis.update.status-penulis', ['id' => ":id"]) }}";
            url = url.replace(":id", user_id);
            $.ajax({
                url: url,
                beforeSend: function(){
                    $('#ok_button').text('Sedang Diproses....');
        ...
        $(document).on('click', '.update-password', function(){
            ...
            $('#password_penulis_form')[0].reset();
            $('#password_penulis_id').val(id);
            $('#updatePasswordModal').modal('show');
        });

        $('#password_penulis_form').on('submit', function(e){
            e.preventDefault();
            $.ajax({
                url: "{{ route('razen-blog.managemen-akun.penulis.update.password') }}",
                method: "POST",
                data: new FormData(this),
                dataType: "json",
                contentType: false,
