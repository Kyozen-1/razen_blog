...
<script>
    const settings = new Settings({
        @if (Auth::guard('penulis')->check())
            attributes: {
                color: "{{Auth::guard('penulis')->user()->color_layout?Auth::guard('penulis')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('penulis')->user()->nav_color?Auth::guard('penulis')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('penulis')->user()->behaviour?Auth::guard('penulis')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('penulis')->user()->layout?Auth::guard('penulis')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('penulis')->user()->radius?Auth::guard('penulis')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('penulis')->user()->placement?Auth::guard('penulis')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('razen-blog.penulis.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
